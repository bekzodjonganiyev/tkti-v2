const SelectLang = {
    uz:[
        {
            title:'Stipendiyalar',
            content:`Universitetning iqtidorli stipendiyalari eng faol talabalarga beriladi, ular o'qishlarida, tadqiqotlarida yoki badiiy loyihalarida ajoyib natijalarga erishadilar, TKTI nomini targ'ib qiluvchi biznes, innovatsiyalar va ijtimoiy faoliyat sohalarida darsdan tashqari mashg'ulotlarda faol ishtirok etadilar. <br />
            Iste’dod stipendiyasi tanlovida bakalavr va magistratura bosqichining barcha talabalari, shuningdek, integratsiyalashgan va ta’lim bo‘lmagan talabalar ishtirok etishlari mumkin, birinchi kursning birinchi semestridagi bakalavr talabalari bundan mustasno. <br />
            `
        },
        {
            title:`Doktorantura ta'lim`,
            content:`Doktorantura ilmiy tadqiqot va tajriba-konstruktorlik ishlarini mustaqil olib borishga, ilmiy muammolarni hal qilishga qodir, akademik va akademik boʻlmagan muhitda ishlay oladigan olimlarni tayyorlashga moʻljallangan. Doktoranturada oʻqish muddati 3 yil (kunduzgi taʼlim boʻyicha) boʻlib, ularga magistratura yoki unga tenglashtirilgan malakaga ega boʻlgan shaxslar hujjat topshirishlari mumkin. Doktorantura dasturiga tadqiqotlar (30 AKTS), tadqiqot, doktorlik dissertatsiyasini tayyorlash va himoya qilish kiradi. Doktoranturani tamomlagandan va doktorlik dissertatsiyasini himoya qilgandan keyin fan doktori ilmiy darajasi beriladi.`
        },
        {
            title:`Sirtqi ta'lim`,
            content:`Kasbiy ta'lim - bu kasbiy malakani ta'minlaydigan fanlar. Ularga bakalavriat ( birinchi bosqich ) taʼlimini tamomlagan va Universitet tomonidan belgilangan boshqa talablarga javob beradigan shaxslar qabul qilinadi. Universitetda bitta kasbiy ta'lim dasturi mavjud - Pedagogika fanlari , uning hajmi 60 ta o'quv krediti bo'lib, o'qish muddati 1 yil. O'qishlar kasbiy ta'limning yakuniy loyihasi bilan yakunlanadi, bitiruvchilarga ta'lim muassasalarida ishlash huquqini beruvchi pedagog malakasi beriladi.`
        },
        {
            title:`Qo'shma ta'lim`,
            content:`Kasbiy ta'lim - bu kasbiy malakani ta'minlaydigan fanlar. Ularga bakalavriat ( birinchi bosqich ) taʼlimini tamomlagan va Universitet tomonidan belgilangan boshqa talablarga javob beradigan shaxslar qabul qilinadi. Universitetda bitta kasbiy ta'lim dasturi mavjud - Pedagogika fanlari , uning hajmi 60 ta o'quv krediti bo'lib, o'qish muddati 1 yil. O'qishlar kasbiy ta'limning yakuniy loyihasi bilan yakunlanadi, bitiruvchilarga ta'lim muassasalarida ishlash huquqini beruvchi pedagog malakasi beriladi.`
        }
    ],
    ru:[
        {
            title:'Стипендии',
            content:`Талантливые стипендии Университета присуждаются наиболее активным студентам, добившимся выдающихся результатов в учебе, научных или творческих проектах, активно участвующим во внеучебной деятельности в сферах бизнеса, инноваций и общественной деятельности, продвигающих имя КТУ. <br />
            Конкурс Talent Scholarship открыт для всех студентов и аспирантов, а также интегрированных и непреподавательских студентов, за исключением студентов бакалавриата в первом семестре их первого года. <br />
            Размер университетской стипендии для талантов составляет 250 евро в месяц. Общая сумма поддержки, выделенная на один семестр, составляет 1000 евро. <br />
            Конкурс стипендий для талантов 2022-2023: <br />
            Осенний семестр | 9 сентября 2022 г. - 22 сентября 2022 г. <br />
            Весенний семестр | 10 февраля 2023 г. - 23 февраля 2023 г. <br />
            Итоги весеннего семестра 2021-2022 гг. <br />
            Все необходимые документы и инструкции должны быть размещены в АИС. Дополнительная информация и важные документы Дополнительная информация: <br />
            
            люди п. klausk@ktu.lt <br />`
        },
        {
            title:'Докторантура',
            content:`Докторантура предназначена для подготовки ученых, способных самостоятельно проводить научно-исследовательские и опытно-конструкторские работы, решать научные задачи, работать в академической и неакадемической среде. Продолжительность обучения в докторантуре составляет 4 года (дневная форма), и могут подавать заявки соискатели со степенью магистра или ее эквивалентом. Программа докторантуры включает обучение (30 кредитов ECTS), исследование, подготовку и защиту докторской диссертации. После окончания докторантуры и защиты докторской диссертации присуждается ученая степень доктора наук.`
        },
        {
            title:`Заочное образование`,
            content:`Профессиональное образование – это предметы, дающие профессиональные навыки. К ним принимаются лица, окончившие степень бакалавра (первая ступень) и соответствующие другим требованиям, установленным университетом. В университете действует одна программа профессионального образования – «Педагогические науки», ее объем составляет 60 кредитов, срок обучения – 1 год. Учеба завершается итоговым проектом профессионального образования, выпускникам присваивается педагогическая квалификация, дающая право работать в образовательных организациях.`
        },
        {
            title:`Совместное обучение`,
            content:`Профессиональное образование – это предметы, дающие профессиональные навыки. К ним принимаются лица, окончившие степень бакалавра (первая ступень) и соответствующие другим требованиям, установленным университетом. В университете действует одна программа профессионального образования – «Педагогические науки», ее объем составляет 60 кредитов, срок обучения – 1 год. Учеба завершается итоговым проектом профессионального образования, выпускникам присваивается педагогическая квалификация, дающая право работать в образовательных организациях.`
        }
    ],
    en:[
        {
            title:'Scholarships',
            content:`The University's Talented Scholarships are awarded to the most active students who achieve outstanding results in their studies, research or artistic projects, actively participate in extracurricular activities in the areas of business, innovation and social activities that promote the name of TKTI. <br />
            The Talent Scholarship competition is open to all undergraduate and graduate students, as well as integrated and non-teaching students, with the exception of undergraduate students in the first semester of their first year. <br />
            The amount of the University's Talent Scholarship is 250 EUR per month. The total amount of support allocated for one semester is 1000 euros. <br />
            2022-2023 Talent Scholarship Contest: <br />
            Fall semester | September 9, 2022 - September 22, 2022 <br />
            Spring Semester | February 10, 2023 - February 23, 2023 <br />
            Results of the spring semester 2021-2022. <br />
            All necessary documents and instructions must be placed in AIS. Additional information and important documents More information: <br />
         
            people p. klausk@ktu.lt <br />`
        },
        {
            title:'Doctoral education',
            content:`The doctoral course is designed to prepare scientists who are able to independently conduct scientific research and experimental design work, solve scientific problems, and work in academic and non-academic environments. The duration of doctoral study is 4 years (full-time), and applicants with a master's degree or equivalent can apply. The doctoral program includes studies (30 ECTS), research, preparation and defense of a doctoral thesis. After completing the doctoral studies and defending the doctoral dissertation, the scientific degree of Doctor of Science is awarded.`
        },
        {
            title:`External education`,
            content:`Vocational education is the subjects that provide vocational skills. Those who have completed bachelor's degree (first stage) and meet other requirements set by the University are admitted to them. The university has one professional education program - Pedagogical Sciences, its volume is 60 credits and the study period is 1 year. The studies end with the final project of professional education, the graduates are given the pedagogic qualification, which gives them the right to work in educational institutions.`
        },
        {
            title:`Joint education`,
            content:`Vocational education is the subjects that provide vocational skills. Those who have completed bachelor's degree (first stage) and meet other requirements set by the University are admitted to them. The university has one professional education program - Pedagogical Sciences, its volume is 60 credits and the study period is 1 year. The studies end with the final project of professional education, the graduates are given the pedagogic qualification, which gives them the right to work in educational institutions.`
        }
    ]
}

export default SelectLang;