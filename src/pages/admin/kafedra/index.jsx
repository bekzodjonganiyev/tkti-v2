...
export const Kafedra = () => {
  ...
  const getData = async () => {
    const res = await apiClientWithFetch.get("Kafedra_data/all");
    if (res.status === 200) {
      setData({
        loading: false,
        data: res.data,
  ...
  const deleteData = async (id) => {
    ...
    const res = await apiClientWithFetch.delete(`Kafedra_data/${id}`);
    if (res.status === 200) {
    ...
    },
    {
      ...
      render: (_, p) => (
        <div className="flex gap-4">
          <Link to={`/adminPanel/kafedra/edit/${p.id}`}><EditIcon /></Link>
          <Popconfirm
