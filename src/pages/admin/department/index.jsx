...
export const Department = () => {
  ...
  const getData = async () => {
    const res = await apiClientWithFetch.get("bm_data/all");
    if (res.status === 200) {
      setData({
        loading: false,
        data: res.data,
  ...
  const deleteData = async (id) => {
    ...
    const res = await apiClientWithFetch.delete(`bm_data/${id}`);
    if (res.status === 200) {
    ...
    },
    {
      ...
      render: (_, p) => (
        <div className="flex gap-4">
          <Link to={`/adminPanel/department/edit/${p.id}`}><EditIcon /></Link>
          <Popconfirm
